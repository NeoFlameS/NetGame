...
void InitObject(ObjectGroup* server_ob)
{
	...
	ObjectCreate(45, 45, server_ob);// 원하는개수넣기.일단 99,99넣음
...
void ObjectCreate(int res, int obs, ObjectGroup* server_ob)
{
	...
	for (int i = 0; i < res; i++)
	{
		...
		server_ob->food[globalfoodIndex++] = *pRes; //500개배열에 넣어준다.
	...
	for (int i = 0; i < obs; i++)
	{
		...
		server_ob->obs[globalResourceIndex++] = *pObs;//500개배열에 넣어준다.
...
bool ObjStateCheck(ObjectGroup* server_ob)
{
	...
	if (server_ob->foodCnt < foodCnt)
	{
		...
		//100개 50개사라짐 50개만큼 뺴서 난수 0으로 초기화 
		...
		ObjectCreate(30, 0, server_ob); //개수만큼늘림.
	}
	if (server_ob->obsCnt < fObstacleCnt)
	{
		...
		ObjectCreate(0, 30, server_ob); //개수만큼늘림.
	}
...
{
	int Xarea = 100; // x영역 범위지정 
	int Yarea = 100; // y영역 범위지정
